...
from flask import current_app, make_response, jsonify, session, request
...
def query_set(queryset, model: db.Model, paginated: bool = False):
...
def login_required(func):
    ...
    def wrapper_function(*args, **kwargs):
        ...
        # Token matching is validated when it is a different endpoint than token 
        if not token:
        ...
        if request.url.split("/")[-1] != "token":
            try:
                ...
                return make_response(jsonify({"error": "Missing bearer token"}), 400)
